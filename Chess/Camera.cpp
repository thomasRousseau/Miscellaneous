...
Camera::Camera () {
  ...
  trackball (curquat, 0.0, 0.3, 0.0, 0.0);
  x = y = z = 0.0;
  _zoom = 3.0;
...
trackball(float q[4], float p1x, float p1y, float p2x, float p2y)
{
    ...
    vset(p2,p2x,p2y,tb_project_to_sphere(TRACKBALLSIZE,p1x,p1y));
...
void mult(float m[4][4], float q[3], float a[3]){
  a[0] = m[0][0]*q[0] + m[0][1]*q[1] + m[0][2]*q[2];
  a[1] = m[1][0]*q[0] + m[1][1]*q[1] + m[1][2]*q[2];
  a[2] = m[2][0]*q[0] + m[2][1]*q[1] + m[2][2]*q[2];
}
