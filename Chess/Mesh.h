...
public:
    inline Vertex () {}
    inline Vertex (Vertex * v):p(Vec3f(v->p[0], v->p[1], v->p[2])),n(Vec3f(v->n[0], v->n[1], v->n[2])){}
...
public:
    ...
    void update_alt_offset();
    /// recompute the minimum and maximum altitude of the mesh
    ...
    /// update the size of the object by given a factor
    void scale(float factor);

    /// rotate the object along the axe and with the angle
    void rotate(Axe, float);
};
